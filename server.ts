...
async function startServer() {
  ...
  app.get('/api/commercial/orders', (req, res) => {
    // Note: Commercial needs to see stock_quantity_mock, so we send the full product data here
    res.json(mockOrders);
  });
  ...
  app.delete('/api/commercial/categories/:id', (req, res) => {
    ...
    // Note: In a real app we'd also handle products belonging to this category.
    res.json({ success: true });
  });
